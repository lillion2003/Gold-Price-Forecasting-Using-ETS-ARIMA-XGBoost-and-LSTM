...
2.  จากการศึกษาหลายๆครั้งพบว่า LSTM มีความอ่อนไหวต่อค่าไฮเพอร์พารามิเตอร์อย่างมาก ซึ่งอาจเป็นเพราะ **โดยทั่วไปแล้วโมเดล Deep Learning จะเหมาะสำหรับข้อมูลจำนวนมาก** ในขณะที่ XGBoost พบว่าความแม่นยำค่อนข้างเสถียรอาจเป็นเพราะแม้ว่าข้อมูลมีจำนวน้อยแต่มีความเพียงพอต่อการฝึกโมเดล

3. **ข้อมูลในสถานการณ์นี้มีจำนวนจำกัด** อาจทำให้ผลลัพธ์ของวิธี Machine Learning ยังไม่แสดงศักยภาพเต็มที่

4. **การออกแบบ Validation** ในการศึกษานี้พยายามออกแบบการ Validation ให้เหมือนตอนพยากรณ์ Test set มากที่สุด ด้วยการนำ Multi-Step Direct ไปใช้ในการประเมิน และกำหนดช่วงเวลาประเมินในแต่ละ Fold = 155 วัน (เท่ากับวันที่ต้องพยากรณ์ Test set) ซึ่งทำให้การ Rolling Window จำนวน 5 Fold จำเป็นต้องออกแบบให้ช่วงเวลามีการซ้อนทับกัน อาจทำให้ผลของแต่ละ Fold มีความสัมพันธ์ต่อกัน (ข้อมูลไม่เพียงพอสำหรับการแบ่งแต่ละ Fold ให้แยกจากกัน)
...
5. **ผลลัพธ์ของวิธี Machine Learning** มีความแตกต่างชัดเจนระหว่างความแม่นยำของการ Validation และการประเมินบน Test set ควรมีการพิจารณาเพิ่มข้อมูลหรือปรับช่วงการ Grid Search

6. **ชุดไฮเพอร์พารามิเตอร์** ที่นำมาใช้ในการศึกษานี้ได้กำหนดให้มีความแตกต่างเบื้องต้นในแต่ละรอบการหา ไม่สามารถกำหนดให้มีความหลากหลายขึ้นได้เนื่องจากขาดทรัพยากรในการคำนวณ ควรมีการขยายช่วงการหาไฮเพอร์พารามิเตอร์เพื่อให้ผลลัพธ์ที่ได้สมบูรณ์ยิ่งขึ้น
...
- **Ensemble Methods**: อาจพิจารณาผสมผสานวิธีต่างๆ เข้าด้วยกัน (Ensemble) เช่น นำ residual จากผลการทำนายของ XGBoost มา fit เด้วย Machine Learning อื่นๆ เพื่อลดค่า Residual เพิ่มเติม ซึ่งเป็นการนำ 2 โมเดลมาใช้ร่วมกัน

- **Real-time Implementation**: สามารถนำตัวแบบที่ดีที่สุดไปพัฒนาเป็นระบบพยากรณ์ราคาทองคำ

- **Data Collection**: เก็บรวบรวมข้อมูลเพิ่มเติมเพื่อปรับปรุงประสิทธิภาพของวิธี Machine Learning

- **Exogenous Variables**: ควรเพิ่มตัวแปรภายนอก เช่น อัตราแลกเปลี่ยน ราคาน้ำมัน หรืออัตราดอกเบี้ย โดยเปลี่ยนไปใช้โมเดล ARIMAX แทนโมเดล ARIMA หรือและเพิ่ม features ในโมเดล Machine Learning เพื่อช่วยเพิ่มความแม่นยำในการพยากรณ์
